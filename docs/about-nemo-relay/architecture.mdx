...
7. Non-streaming LLM lifecycle completion hooks observe success or failure and
   can finish owned work, such as configured memory write-back.
...
in-memory reference implementation, and opt-in automatic recall/write-back for
ordinary non-streaming managed LLM calls. Python exposes the native reference
component from `nemo_relay.memory`; Node.js exposes it from
`nemo-relay-node/memory`. Refer to the generated
...
this shipped vertical slice from future queues, adapters, cache coordination,
evaluation, and UI work.
...
| `crates/memory` | Provider-neutral memory runtime, deterministic in-memory provider, conformance helpers, and opt-in automatic managed-LLM component. |
