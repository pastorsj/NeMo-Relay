...
Managed tool and LLM execution follows the same high-level order. Non-streaming
LLM calls also have an asynchronous lifecycle seam used by components such as
automatic memory:

1. Conditional-execution guardrails decide whether work can proceed.
2. Request intercepts can rewrite the real request.
3. Non-streaming LLM lifecycle preparation hooks can asynchronously transform
   the request before its start event.
...
7. Non-streaming LLM lifecycle completion hooks observe success or failure and
   can finish owned work or admit it to a component-owned bounded queue, such
   as configured memory write-back.
8. Sanitize-response guardrails can rewrite the emitted end-event payload.
...
# Add the opt-in automatic managed-LLM component
...
The shipped memory surface includes a direct Rust provider API, deterministic
in-memory provider and maintainer, bounded local work queue, and opt-in
automatic recall/write-back for ordinary non-streaming managed LLM calls.
Write-back is inline by default and can use explicit background delivery.
Python exposes the native reference component and async queue controls from
`nemo_relay.memory`; Node.js exposes them from `nemo-relay-node/memory`. Refer
to the generated
[`nemo-relay-memory` library reference](/reference/api/rust-library-reference/nemo-relay-memory)
for its provider, runtime, conformance, and automatic-component symbols. The
[memory runtime design and status](/contribute/memory-runtime-design) separates
the shipped local async lifecycle from future durable queues, provider adapters,
cache coordination, evaluation, and UI work.
...
| `crates/memory` | Provider-neutral memory runtime, deterministic provider and maintainer, bounded work queue, conformance helpers, and opt-in automatic managed-LLM component. |
