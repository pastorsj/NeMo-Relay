...
following remain future work in Relay: durable/distributed queues and
arbitrary Python/Node provider bridges into native automatic execution.
Provider-specific adapters, Hivemind integration, attribution and evaluation,
and the agent UI demonstration live in a separate companion demonstration
repository. Streaming managed LLM calls reject visible lifecycle hooks instead
of applying partial memory behavior.
...
The Python `MemoryProvider` contract remains the language-level adapter
boundary. Provider-specific implementations live in companion application or
integration repositories and cannot yet be passed to the native automatic
component. The Node contract has the same boundary.
...
maintainer lifecycle. Provider-specific adapters live in application or
...
remain external adapters or services.
...
| `nemo-relay-memory` | Shipped async provider contract, reference provider and maintainer, bounded local queue, automatic policy, deterministic selection, write projection, and conformance tests. |
...
| Companion demo repository | Provider adapters, pinned memory-service configurations, demo backend and agent, scripted recall scenario, evaluation output, and a thin evidence UI. |
...
evidence clearly. The companion project is developed in its own repository and
...
The companion demonstration project uses the first five profiles to exercise
different capability shapes. These are integration profiles, not features or
dependencies of the `nemo-relay` packages. A profile can be useful without
implementing fake CRUD parity.
...
The companion evaluation harness will combine retrieval tests, end-to-end answer
...
6. **Validated in the companion project:** Add Mem0, Hindsight, Graphiti OSS,
   Cognee, and Hivemind profiles without adding vendor dependencies to Relay.
7. **Validated in the companion project:** Add a pinned evaluation harness and
   publish raw per-metric results.
8. **Validated in the companion project:** Build the scripted two-session agent
   and thin evidence UI showing automatic recall, storage, evidence states, and
