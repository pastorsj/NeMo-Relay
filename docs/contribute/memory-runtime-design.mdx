...
- Optional direct Python adapters for Mem0 OSS 2.x, Hindsight 0.8, Graphiti OSS
  0.29, and Cognee 1.2 with a shared supported-conformance report and no
...
- Graphiti episode/fact retrieval with current temporal edge evidence, plus
  Cognee forget and Consolidate-through-improve capability profiles
...
provider bridges into automatic execution, the Hivemind adapter, attribution
and evaluation, and the agent UI demonstration. Streaming managed
...
6. **In progress:** Mem0, Hindsight, Graphiti OSS, and Cognee are complete;
   Hivemind remains.
