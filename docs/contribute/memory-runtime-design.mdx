...
This proposal defines the remaining automatic lifecycle and integration work.
It builds on the current [Relay architecture](/about-nemo-relay/architecture),
...
The direct provider boundary from Phase 2 is implemented. The shipped surface
...
- A deterministic, network-free in-memory provider and reusable conformance
  report
- Contract-only Python and Node.js entry points at `nemo_relay.memory` and
  `nemo-relay-node/memory`

This implementation does not install memory into the managed agent lifecycle.
Automatic retrieval and write-back, plugin activation, ATOF evidence,
background maintenance, cache coordination, OSS adapters, evaluation, and the
agent UI demonstration remain proposed work. Constructing `MemoryRuntime`
provides direct provider access only.
...
isolation. A demo can explicitly choose fail-open behavior. Concurrent-isolation
...
The following diagram shows the proposed hot path and asynchronous handoff.
...
The implementation will extend the generic mark-event surface to accept
optional `EventCategory`, `CategoryProfile`, and `DataSchema` values. It will
not add a memory-only event bypass.
...
parent. Evaluation can also use the existing evaluator scope category.
