...
The Python `MemoryProvider` contract now has direct Mem0 OSS, Hindsight,
Graphiti OSS, and Cognee implementations. Those language-defined objects cannot
yet be passed to the native automatic component. The Node contract remains an
adapter boundary.
...
6. **Complete in the companion project:** Add Mem0, Hindsight, Graphiti OSS,
   Cognee, and Hivemind profiles without adding vendor dependencies to Relay.
7. Add the pinned evaluation harness and publish raw per-metric results in the
   companion project.
8. Build the companion scripted two-session agent and thin evidence UI
