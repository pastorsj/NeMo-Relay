...
description: "Shipped provider and automatic-memory behavior plus the staged design for queues, adapters, cache coordination, evaluation, and a demonstration."
position: 6
---
...
The provider boundary and automatic reference vertical slice are implemented.
...
- Deterministic automatic recall, codec-aware tagged injection, synchronous
  projected write-back, and references-only categorized evidence
...
following remain future work: background queues and maintenance, arbitrary
Python/Node provider bridges, OSS adapters, prompt-cache coordination,
...
The Python and Node `MemoryProvider` contracts describe future adapter
implementations. Those language-defined objects cannot yet be passed to the
native automatic component.
...
implementation, conformance helpers, and automatic component. Later phases add
provider factories, a bounded queue, and background lifecycle to the same crate.
The existing plugin system validates, initializes, reports, replaces, and
clears components; future queue drain and provider shutdown reuse that lifecycle
...
| `nemo-relay-memory` | Shipped async provider contract, reference provider, automatic policy, deterministic selection, write projection, and conformance tests; future provider factories, bounded queue, and maintainers. |
...
call continues without memory if the provider is unavailable. Shipped storage
is awaited and defaults to fail open. A later bounded queue will make overflow
and delivery failures observable. Memory processing preserves the managed
callback's result unless an explicit fail-closed policy says otherwise.
...
untrusted context and prepends it to the triggering user content. Stable system
and developer instructions remain ahead of that volatile block. The callback
and LLM start event receive the same prepared request. Write projection retains
the original user content and, by default, normalized assistant content; it
excludes tagged injected blocks to prevent recursive memory pollution.
...
then response sanitization/end-event emission. Phase 3 awaits the provider store
inside lifecycle completion before returning the callback result. Retrieval and
storage have independent fail-open/fail-closed policies, and providers own any
retry behavior.
...
The following diagram separates the shipped hot path from the future background
handoff.
...
flowchart LR
    ...
    Complete --> Store[Await projected provider store]
    Store --> Result[Return callback result]
    Complete -. future bounded handoff .-> Queue[Background queue]
    Queue -. future .-> Async[Evaluation and maintenance]
    Async -. derived writes .-> Store
    ...
    class Queue,Async blue-lightest;
```
...
`nemo.relay.memory.operation` version `0.1`. Marks carry status, policy,
...
Memory can use Adaptive Prompt IR for codec-aware block placement, token
