...
Proposed contracts are reviewed separately from the shipped shared contract
pages and must not be used as API reference. They record decisions for planned
work until implementation, binding parity, and validation establish a supported
surface.

- [Memory Runtime Design Proposal](/contribute/memory-runtime-design) records
  provider, identity, lifecycle, observability, asynchronous maintenance,
  cache, evaluation, and repository-boundary decisions for planned work.
